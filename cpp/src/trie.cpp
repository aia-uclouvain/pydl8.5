...
TrieNode *Trie::find(
        Array<Item> itemset) { ///seek itemset in the trie from root. Return null if not exist and the node of the last item if it exists
    TrieNode *p = root, *p2;
...
TrieNode *Trie::insert(
        Array<Item> itemset) { /// insert itemset. Check from root and insert items only they do not exist using createTree
    TrieNode *p = root, *p2;
    vector<TrieEdge>::iterator t, e;
