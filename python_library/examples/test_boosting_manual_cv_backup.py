...
N_FOLDS, MAX_DEPTH, MIN_SUP, MAX_TREES = 5, 3, 1, 0
...
for k in range(N_FOLDS):
    X_train, y_train, X_test, y_test = X_trains[k], y_trains[k], X_tests[k], y_tests[k]
    clf = DL85Booster(max_depth=MAX_DEPTH, min_sup=MIN_SUP, time_limit=300, regulator=0.4,
                      max_estimators=MAX_TREES, print_output=True, quiet=False)
    clf.fit(X_train, y_train)
    preds = clf.predict(X_test)
