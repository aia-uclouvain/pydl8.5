...
file_out = open("../output/out_optiboost_md_ada_grid_" + ((str(MAX_ITERATIONS) + '_') if MAX_ITERATIONS > 0 else '') + "depth_" + str(MAX_DEPTH) + "_none.csv", "a+")
#directory = '../datasets'
...
for filename in files[files.index(first_file):]:
    ...
    if True:
        ...
        #
        #     print("Fold", k+1, "- Search for the best regulator using grid search...", MAX_TREES)
        #     gd_sr = GridSearchCV(estimator=AdaBoostClassifier(base_estimator=DecisionTreeClassifier(max_depth=MAX_DEPTH)), error_score=np.nan,
        #                          param_grid=parameters_ada, scoring='accuracy', cv=custom_cv, n_jobs=-1, verbose=0)
        #     gd_sr.fit(X_train, y_train)
        #     print()
        ...
        print("Adaboost n_trees grid search")
        print("Dataset :", filename)
        n_trees, fps, fns, fit_times, train_scores, test_scores, n_iter, regulators, gammas, n_opti, n_nodes = [], [], [], [], [], [], [], [], [], 0, []
        for k in range(N_FOLDS):
            X_train, X_test, y_train, y_test = X_trains[k], X_tests[k], y_trains[k], y_tests[k]
        ...
        print("Adaboost n_trees learning rate grid search")
        print("Dataset :", filename)
        n_trees, fps, fns, fit_times, train_scores, test_scores, n_iter, regulators, gammas, n_opti, n_nodes = [], [], [], [], [], [], [], [], [], 0, []
        for k in range(N_FOLDS):
            X_train, X_test, y_train, y_test = X_trains[k], X_tests[k], y_trains[k], y_tests[k]
        ...
        print("Adadl85 n_trees grid search")
        print("Dataset :", filename)
        n_trees, fps, fns, fit_times, train_scores, test_scores, n_iter, regulators, gammas, n_opti, n_nodes = [], [], [], [], [], [], [], [], [], 0, []
        for k in range(N_FOLDS):
            X_train, X_test, y_train, y_test = X_trains[k], X_tests[k], y_trains[k], y_tests[k]
            ...
            print("Fold", k+1, "- Search for the best regulator using grid search...", MAX_TREES)
            gd_sr = GridSearchCV(estimator=AdaBoostClassifier(base_estimator=DL85Classifier(max_depth=MAX_DEPTH)), error_score=np.nan,
                                 param_grid=parameters_ada_1, scoring='accuracy', cv=custom_cv, n_jobs=-1, verbose=0)
            gd_sr.fit(X_train, y_train)
            print()
            print("Fold", k+1, "- Running cross validation for LPBoost + DL8.5 with best regulator =", -1, "on", filename)
        ...
        print("Adadl85 n_trees learning rate grid search")
        print("Dataset :", filename)
        n_trees, fps, fns, fit_times, train_scores, test_scores, n_iter, regulators, gammas, n_opti, n_nodes = [], [], [], [], [], [], [], [], [], 0, []
        for k in range(N_FOLDS):
            X_train, X_test, y_train, y_test = X_trains[k], X_tests[k], y_trains[k], y_tests[k]
        ...
        clf_results = cross_validate(estimator=DL85Boostera(max_depth=MAX_DEPTH, regulator=regg, max_iterations=-1, quiet=True, gamma=None), X=X, y=y, scoring='accuracy',
