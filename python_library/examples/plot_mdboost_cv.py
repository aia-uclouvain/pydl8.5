...
directory = '../datasets'
file_out = open("output/out_mdb_maxiter_" + (str(MAX_ITERATIONS) if MAX_ITERATIONS > 0 else 'none') + "_depth_" + str(MAX_DEPTH) + ".csv", "a+")
files = ['zoo-1.txt',
         'hepatitis.txt',
         'lymph.txt',
         'audiology.txt',
...
for filename in files[files.index(first_file):files.index(first_file)+1]:
    ...
    parameters = {'regulator': [1, 10, 50, 100, 1000]}
    for name, base_clf in zip([x + " + " + y for x in ["MDBOOST"] for y in ["DL85", "CART"]], [DL85Booster(base_estimator=base, max_depth=MAX_DEPTH, model=mod, max_iterations=MAX_ITERATIONS) for mod in [MODEL_QP_MDBOOST] for base in [None, DecisionTreeClassifier(max_depth=MAX_DEPTH)]]):
        print("Optiboost + {}".format(name))
        print("Dataset :", filename)
        file = open("output/" + name.replace(" + ", "_") + "_mdb_maxiter_" + (str(MAX_ITERATIONS) if MAX_ITERATIONS > 0 else 'none') + "_depth_" + str(MAX_DEPTH) + ".csv", "a+")
