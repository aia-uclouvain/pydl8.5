...
for filename in sorted(os.listdir(directory)):
    if filename.endswith(".txt") and not filename.startswith("paper"):
        ...
        for k in range(N_FOLDS):
            ...
            print("Fold", k+1, "- Search for the best regulator using grid search...the best regulator is used to fit an estimator on the whole dataset", MAX_TREES)
            ...
            n_trees.append(gd_sr.best_estimator_.n_estimators_)
            fit_times.append(gd_sr.best_estimator_.duration_)
            train_scores.append(accuracy_score(y_train, gd_sr.best_estimator_.predict(X_train)))
            test_scores.append(accuracy_score(y_test, y_pred))
            n_iter.append(gd_sr.best_estimator_.n_iterations_)
            regulators.append(gd_sr.best_params_["regulator"])
            n_opti = n_opti + 1 if gd_sr.best_estimator_.optimal_ else n_opti
            ...
            to_write += [n_iter[-1], n_trees[-1], fit_times[-1], gd_sr.best_estimator_.optimal_, train_scores[-1], test_scores[-1], fps[-1], fns[-1], regulators[-1]]
        ...
        for k in range(N_FOLDS):
            ...
            gd_sr = GridSearchCV(estimator=DL85Booster(base_estimator=DecisionTreeClassifier(max_depth=MAX_DEPTH,
                                 ...
                                 param_grid=parameters, scoring='accuracy', cv=N_FOLDS_TUNING, n_jobs=-1, verbose=VERBOSE_LEVEL)
            gd_sr.fit(X_train, y_train, X_add=X_train, y_add=y_train, valid=False, data_name=filename + ".csv")
            print()
