        ...
        if (next_attributes.size == 0) {
            ...
            *nodeError = leafError;
            ...
            forEach (i, next_attributes) {
                ...
                itemsets[0] = addItem(itemset, items[0]);
                ...
                itemsets[1] = addItem(itemset, items[1]);
                ...
                cover->intersect(next_attributes[i], first);
                nodes[first] = recurse(itemsets[first], items[first], nodes[first], next_attributes, cover, depth + 1, child_ub, llb);
                addInfoForLowerBound(cover, nodes[first]->data, errlb1, errlb2, errlb3, covlb1, covlb2, covlb3, sclb1, sclb2, sclb3, sflb1, sflb2, sflb3, suplb);
                ...
                if (query->canimprove(nodes[first]->data, child_ub)) {
                    ...
                    cover->intersect(next_attributes[i], !first);
                    float remainUb = child_ub - leftError;
                    nodes[!first] = recurse(itemsets[!first], items[!first], nodes[!first], next_attributes, cover, depth + 1, remainUb, rlb);
                    addInfoForLowerBound(cover, nodes[!first]->data, errlb1, errlb2, errlb3, covlb1, covlb2, covlb3, sclb1, sclb2, sclb3, sflb1, sflb2, sflb3, suplb);
                    ...
                    bool hasUpdated = query->updateData(node->data, child_ub, next_attributes[i], nodes[first]->data, nodes[!first]->data);
...
Error LcmPruned::computeLowerBound(RCover* cover, bitset<M>* covlb1, bitset<M>* covlb2, bitset<M>* covlb3,
    ...
    if (covlb1){
        ...
        forEachClass(n){
            c += max(0, sclb1[n] - dif1[n]);
            f += max(0, sflb1[n] - dif1[n]);
        }
        tmp1 = min(c, f);
        deleteSupports(dif1);
    }
    ...
    if (covlb3){
        ...
        forEachClass(n){
            c += max(0, sclb3[n] - dif3[n]);
            f += max(0, sflb3[n] - dif3[n]);
        }
        tmp3 = min(c, f);
        deleteSupports(dif3);
    }
...
void LcmPruned::addInfoForLowerBound(RCover* cover, QueryData * node_data, Error errlb1, Error errlb2, Error errlb3,
    ...
    if (((QueryData_Best *) node_data)->error < FLT_MAX){
        Error b = (((QueryData_Best *) node_data)->error < FLT_MAX) ? ((QueryData_Best *) node_data)->error : ((QueryData_Best *) node_data)->lowerBound;
