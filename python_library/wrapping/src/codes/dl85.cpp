...
string
search(int argc, char *argv[], Supports supports, Transaction ntransactions, Attribute nattributes, Class nclasses,
       Bool *data, Class *target, float maxError, bool stopAfterError, bool iterative) {
    /*cout << "argc = " << argc << endl;
    cout << "argv = " ;
    ...
    if (argc < 3) {
        ...
        if (maxError < 0)
            query = new Query_TotalFreq(trie, dataReader, experror, timeLimit, continuous);
        else
            query = new Query_TotalFreq(trie, dataReader, experror, timeLimit, continuous, maxError, stopAfterError);
        ...
        if (iterative) {
            lcm = new LcmIterative(dataReader, query, trie, infoGain, infoAsc, allDepths);
            ((LcmIterative *) lcm)->run();
        } else {
            lcm = new LcmPruned(dataReader, query, trie, infoGain, infoAsc, allDepths);
            ((LcmPruned *) lcm)->run();
        }
