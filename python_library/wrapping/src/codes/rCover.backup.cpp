...
Supports RCover::getClassSupport(){
    Supports classSupport = newSupports();
...
Error RCover::getWeightedError(vector<float>& weights) {
    float* itemsetSupport = new float[nclasses];
    forEachClass(c) itemsetSupport[c] = 0;
    Error sum_error = 0;
    Error max_error = 0;
    for (int j = 0; j < nclasses; ++j) {
        bitset<M> * classCover = dm->getClassCover(j);
        for (int i = 0; i < limit.top(); ++i) {
            ...
            vector<int> tids = ::getTransactionsID(coverWords[validWords[i]].top() & classCover[validWords[i]], real_word_index);
            for (auto tid : tids){
                itemsetSupport[j] += weights[tid];
            }
        }
        sum_error += itemsetSupport[j];
        max_error = (itemsetSupport[j] > max_error) ? itemsetSupport[j] : max_error;
    }
