...
public:
    ...
    public:
        ...
        u_int getFirstSetBitPos(u_long n)
        {
            return log2(n & -n) + 1;
        }
...
public:
    ...
    Supports minusMe(ulong* cover1);

    SupportClass countDif(ulong* cover1);

    ulong* getTopBitsetArray() const;

    Support getSupport();

    virtual Supports getSupportPerClass() = 0;

    virtual SupportClass countSupportClass(ulong coverWord, int wordIndex) = 0;

    virtual Supports getSupportPerClass(ulong* cover, int nValidWords, int* validIndexes) = 0;
    ...
    public:
        ...
        void setNextTransID() {
            if (wordIndex < container->limit.top()) {
                int indexForTransactions = container->nWords - (container->validWords[wordIndex]+1);
                int pos = getFirstSetBitPos(word);
