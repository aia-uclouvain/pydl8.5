...
bool sortDecOrder(const pair<TrieNode*,TrieNode*> &pair1, const pair<TrieNode*,TrieNode*> &pair2) {
...
Node *Cache_Trie::get(Itemset &itemset) {
...
//     cout << "cachesize before wipe = " << getCacheSize() << endl;
    ...
    for (auto it = heap.rbegin(); it != heap.rend(); it++) {
        ...
        it->second->edges.erase(find_if(it->second->edges.begin(), it->second->edges.end(), [it](const TrieEdge &look) { return look.subtrie == it->first; }));
        // remove the node
        delete it->first;
        heap.pop_back();
        counter++;
